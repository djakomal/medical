...
@GetMapping("/docteur/image/{id}")
public ResponseEntity<byte[]> getDocteurImage(@PathVariable Long id) {

    try {
        byte[] imageBytes = authService.getDocteurImage(id);
        ...
        return ResponseEntity.notFound().build(); // IMPORTANT
    }
}
